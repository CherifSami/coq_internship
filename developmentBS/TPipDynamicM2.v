...
Module StaticInductC := StaticInduct IdT.
Export StaticInductC.
...
| BindMS_Cg_EStep : forall (fenv fenvL fenv': funEnv)
                           (env envL env': valEnv) 
                           (n n': W) (e e': Exp),
    fenv' = overrideE fenvL fenv -> 
    env' = overrideE envL env ->
...
| Apply_EStep1 : forall (fenv fenv': funEnv) (env env': valEnv)
     ...
     env' = mkVEnv pt vs ->
     EStep fenv env (Conf Exp n
              (Apply (QF (FC fenv' pt e0 e1 x (S i))) (PS es)))
                    (Conf Exp n (BindMS (updateE fenv' x
              (FC fenv' pt e0 e1 x i)) env' e1))
...
(*
Module DynamicC := Dynamic IdMod1.
