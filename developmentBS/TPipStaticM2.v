...
Instance NatV : ValTyp nat.
  
Instance UnitV : ValTyp unit.

Instance BoolV : ValTyp bool.
...
(** value constructors specialised with semantic types *) 
...
Inductive FunTyping : (** Functions *)
  ...
  | FunS_Typing: forall (ftenv ftenv': funTC) (tenv: valTC)
                        (fenv fenv': funEnv) 
            (e0 e1: Exp) (x: Id) (n: nat) (t: VTyp),
      ftenv' = updateE ftenv x (FT tenv t) -> 
      fenv' = updateE fenv x (FC fenv tenv e0 e1 x n) -> 
      MatchEnvsT FunTyping fenv ftenv -> 
...
with ExpTyping : (** expressions *)
  ...
  | BindMS_Typing : forall (ftenv ftenvP ftenv': funTC)
                           ...
                           (e: Exp) (t: VTyp), 
                       EnvTyping envP tenvP ->
                       MatchEnvsT FunTyping fenv ftenv -> 
                       MatchEnvsT FunTyping fenvP ftenvP ->
                       tenv' = overrideE tenvP tenv ->
                       ftenv' = overrideE ftenvP ftenv ->                       
                       fenv' = overrideE fenvP fenv ->                       
...
| Define_Typing : forall (ftenv ftenv': funTC) (fenv fenv': funEnv)   
                         ...
                         (ft: FTyp) (t: VTyp), 
      FEnvTyping fenv ftenv -> 
      ftenv' = updateE ftenv x ft -> 
      fenv' = updateE fenv x f -> 
      QFunTyping ftenv fenv (QF f) ft ->
...
Definition ExTDefFun
  := ExRelValT FunTyping.

Definition ExTDefFunProj1
  := ExRelValTProj1 FunTyping.
